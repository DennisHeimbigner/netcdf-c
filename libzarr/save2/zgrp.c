...
NCZ_def_grp(int parent_ncid, const char *name, int *new_ncid)
{
    ...
    if ((retval = ncz_find_grp_h5(parent_ncid, &grp, &h5)))
        return retval;
    assert(h5);
...
NCZ_rename_grp(int grpid, const char *name)
{
    ...
    /* Find info for this file and group, and set pointer to each. */
    if ((retval = ncz_find_grp_h5(grpid, &grp, &h5)))
        return retval;
    ...
    if ((retval = ncz_check_dup_name(grp->parent, norm_name)))
        return retval;

    /* If it's not in define mode, switch to define mode. */
    if (!(h5->flags & NC_INDEF))
        if ((retval = NCZ_redef(grpid)))
            return retval;
    ...
    if (ncz_grp->hdf_grpid)
    {
        ...
        /* Close the group */
        if (H5Gclose(ncz_grp->hdf_grpid) < 0)
            return NC_EHDFERR;
        ncz_grp->hdf_grpid = 0;

        /* Attempt to rename & re-open the group, if the parent group is open */
        if (parent_ncz_grp->hdf_grpid)
        {
            /* Rename the group. */
            if (H5Lmove(parent_ncz_grp->hdf_grpid, grp->hdr.name,
                        parent_ncz_grp->hdf_grpid, name, H5P_DEFAULT,
                        H5P_DEFAULT) < 0)
                return NC_EHDFERR;

            /* Reopen the group, with the new name. */
            if ((ncz_grp->hdf_grpid = H5Gopen2(parent_ncz_grp->hdf_grpid, name,
                                                H5P_DEFAULT)) < 0)
                return NC_EHDFERR;
        }
    }
