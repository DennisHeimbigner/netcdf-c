...
/* From nc4mem.c */
extern int NCZ_create_image_file(NC_FILE_INFO_T* h5, size_t);
                ...
                void* parameters, NC *nc)
{
    ...
    if ((retval = ncz_nc4f_list_add(nc, path, (NC_WRITE | cmode))))
        BAIL(retval);
    ncz_info = NCZ_DATA(nc);
    assert(ncz_info && ncz_info->root_grp);
    ncz_info->root_grp->atts_read = 1;

    /* Add struct to hold NCZ-specific file metadata. */
    if (!(ncz_info->format_file_info = calloc(1, sizeof(NCZ_FILE_INFO_T))))
        BAIL(NC_ENOMEM);
    ncz_info = (NCZ_FILE_INFO_T *)ncz_info->format_file_info;

    /* Add struct to hold NCZ-specific group info. */
    if (!(ncz_info->root_grp->format_grp_info = calloc(1, sizeof(NCZ_GRP_INFO_T))))
        return NC_ENOMEM;
    ncz_grp = (NCZ_GRP_INFO_T *)ncz_info->root_grp->format_grp_info;

    ncz_info->mem.inmemory = (cmode & NC_INMEMORY) == NC_INMEMORY;
    ncz_info->mem.diskless = (cmode & NC_DISKLESS) == NC_DISKLESS;
    ncz_info->mem.persist =  (cmode & NC_PERSIST) == NC_PERSIST;
    ncz_info->mem.created = 1;
    ncz_info->mem.initialsize = initialsz;

    /* diskless => !inmemory */
    if(ncz_info->mem.inmemory && ncz_info->mem.diskless)
        BAIL(NC_EINTERNAL);

    if(ncz_info->mem.inmemory && parameters)
        ncz_info->mem.memio = *(NC_memio*)parameters;
#ifdef USE_PARALLEL4
    else if(parameters) {
    ...
    if (!ncz_info->mem.diskless && !ncz_info->mem.inmemory) {
    ...
    if (mpiinfo != NULL) {
        ...
        if (MPI_SUCCESS != MPI_Comm_dup(comm, &ncz_info->comm))
            BAIL(NC_EMPI);
        comm_duped++;
        if (MPI_INFO_NULL != info)
        {
            if (MPI_SUCCESS != MPI_Info_dup(info, &ncz_info->info))
                BAIL(NC_EMPI);
            info_duped++;
        }
        else
    ...
    /* If ZARR supports collective metadata operations, turn them
     * on. This is only relevant for parallel I/O builds of NCZ. */
    ...
    if(ncz_info->mem.inmemory) {
        retval = NCZ_create_image_file(ncz_info,initialsz);
        if(retval)
            BAIL(retval);
    }
    else
        if(ncz_info->mem.diskless) {
            ...
            if (H5Pset_fapl_core(fapl_id, alloc_incr, (ncz_info->mem.persist?1:0)) < 0)
                BAIL(NC_EHDFERR);
            if ((ncz_info->hdfid = H5Fcreate(path, flags, fcpl_id, fapl_id)) < 0)
                BAIL(EACCES);
        }
        else /* Normal file */
        {
    ...
    /* Open the root group. */
    if ((ncz_grp->hdf_grpid = H5Gopen2(ncz_info->hdfid, "/", H5P_DEFAULT)) < 0)
        BAIL(NC_EFILEMETA);
    ...
    /* Save the ZARR superblock number and set the _NCProperties attribute. */
    if ((retval = NCZ_new_provenance(ncz_info)))
        BAIL(retval);
           ...
           const NC_Dispatch *dispatch, NC *nc_file)
{
    ...
    /* If this is our first file, turn off ZARR error messages. */
    if (!ncz_initialized)
        ncz_initialize();
    ...
    res = ncz_create_file(path, cmode, initialsz, parameters, nc_file);
