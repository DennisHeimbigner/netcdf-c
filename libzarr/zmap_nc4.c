...
static int zcreateobj(Z4MAP*, NClist* segments, off64_t, int* objidp);
static int zcreatedim(Z4MAP*, off64_t dimsize, int* dimidp);
...
static int
znc4len(NCZMAP* map, const char* key, off64_t* lenp)
{
    int stat = NC_NOERR;
    Z4MAP* z4map = (Z4MAP*)map;
    NClist* segments = nclistnew();
...
static int
znc4define(NCZMAP* map, const char* key, off64_t len)
{
    int stat = NC_NOERR;
    int grpid;
...
static int
znc4read(NCZMAP* map, const char* key, off64_t start, off64_t count, void* content)
{
    int stat = NC_NOERR;
    int grpid,vid;
    Z4MAP* z4map = (Z4MAP*)map; /* cast to true type */
...
static int
znc4write(NCZMAP* map, const char* key, off64_t start, off64_t count, const void* content)
{
    int stat = NC_NOERR;
    int grpid,vid;
    Z4MAP* z4map = (Z4MAP*)map; /* cast to true type */
...
static int
znc4readmeta(NCZMAP* map, const char* key, off64_t avail, char* content)
{
    int stat = NC_NOERR;
    int grpid;
    Z4MAP* z4map = (Z4MAP*)map; /* cast to true type */
...
static int
znc4writemeta(NCZMAP* map, const char* key, off64_t count, const char* content)
{
    int stat = NC_NOERR;
    int grpid;
    Z4MAP* z4map = (Z4MAP*)map; /* cast to true type */
...
static int
zcreatedim(Z4MAP* z4map, off64_t dimsize, int* dimidp)
{
    int stat = NC_NOERR;
    char name[NC_MAX_NAME];
    int dimid;
