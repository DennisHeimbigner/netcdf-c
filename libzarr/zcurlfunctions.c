...
#define ZBUFFERSIZE "HTTP.READ.BUFFERSIZE"
#define ZKEEPALIVE "HTTP.KEEPALIVE"
...
/* forward */
static int set_curlflag(NCZINFO*, int flag);
static int set_curlopt(NCZINFO*, int flag, void* value);

static int
check(NCZINFO* info, int flag, void* value)
{
...
*/
static int
set_curlopt(NCZINFO* state, int flag, void* value)
{
    int ret = NC_NOERR;
...
*/
static int
set_curlflag(NCZINFO* state, int flag)
{
    int ret = NC_NOERR;
    switch (flag) {
...
int
NCZ_set_flags_perfetch(NCZINFO* state)
{
    int ret = NC_NOERR;
    /* currently none */
    return THROW(ret);
...
int
NCZ_set_flags_perlink(NCZINFO* state)
{
    int ret = NC_NOERR;
...
*/
static int
set_curl_options(NCZINFO* state)
{
    int ret = NC_NOERR;
    NClist* store = NULL;
    int i;
	...
	NCTriple* triple = (NCTriple*)nclistget(store,i);
	  ...
	  if(triple->host != NULL)
        ...
        flag = NCZ_curlflagbyname(flagname);
...
*/
void
NCZ_curl_protocols(NCZINFO* state)
{
...
*/
ncerror
NCZ_get_rcproperties(NCZINFO* state)
{
    ncerror err = NC_NOERR;
    char* option = NULL;
#ifdef HAVE_CURLOPT_BUFFERSIZE
    option = NC_rclookup(ZBUFFERSIZE,state->uri->uri);
...
#endif
#ifdef HAVE_CURLOPT_KEEPALIVE
    option = NC_rclookup(ZKEEPALIVE,state->uri->uri);
    if(option != NULL && strlen(option) != 0) {
...
*/
int
NCZ_set_curlstate(NCZINFO* state, int flag, void* value)
{
    int ret = NC_NOERR;
    switch (flag) {
	...
	if(!NCZ_parseproxy(state,(char*)value))
...
void
NCZ_curl_printerror(NCZINFO* state)
{
    fprintf(stderr,"curl error details: %s\n",state->curl->errdata.errorbuf);
}

CURLcode
NCZ_reportcurlerror(CURLcode cstat)
{
    if(cstat != CURLE_OK) {
