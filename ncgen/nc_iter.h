 ...
 * free(). */
extern int NC_get_iter(Symbol* vsym, size_t bufsize, NCiter_t **iterpp);

/* Iterate over blocks of variable values, using start and count
 * vectors.  Returns number of values to access (product of counts),
 * or 0 if done. */
extern size_t NC_next_iter(NCiter_t *iterp, size_t *start, size_t *count);

/* Release memory allocated for iterator */
extern int NC_free_iter(NCiter_t *iterp);
