...
write_grp(NC_FILE_INFO_T* file, NCZ_FILE_INFO_T* zfile, NCZMAP* map, NC_GRP_INFO_T* grp)
{
    ...
    if(!purezarr) {
        ...
        if((stat = NCJinsert(jgroup,NCZ_V3_GROUP,json))) goto done;
        json = NULL;
    }
...
write_var_meta(NC_FILE_INFO_T* file, NCZ_FILE_INFO_T* zfile, NCZMAP* map, NC_VAR_INFO_T* var)
{
    ...
    */
    ...
    /* compressor key */
    /* From V3 Spec: A JSON object identifying the primary compression codec and providing
...
write_atts(NC_FILE_INFO_T* file, NCZ_FILE_INFO_T* zfile, NCZMAP* map, NC_OBJ* container, NCindex* attlist)
{
    ...
    if(NCJlength(jatts) > 0) {
        if(!purezarr) {
	    /* Insert the NCZ_V3_ATTR attribute */
	    ...
	    if(jtypes != NULL)
            ...
            jtypes = NULL;
	    if(jdict != NULL)
                {if((stat = NCJinsert(jatts,NCZ_V3_ATTR,jdict))) goto done;}
            jdict = NULL;
	}
        /* write .zattrs path */
...
read_superblock(NC_FILE_INFO_T* file, int* nczarrvp)
{
	...
	if((stat = NCJdictget(jblock,NCZ_V3_SUPERBLOCK,&jtmp))) goto done;
	if(jtmp != NULL) {
...
read_vars(NC_FILE_INFO_T* file, NCZ_FILE_INFO_T* zfile, NCZMAP* map, NC_GRP_INFO_T* grp, NClist* varnames)
{
	...
	if(!purezarr) {
	    ...
	    if(!stat && jncvar == NULL)
	        {if((stat = NCJdictget(jvar,NCZ_V3_ARRAY_UC,&jncvar))) goto done;}
...
NCZ_load_jatts(NCZMAP* map, NC_OBJ* container, NCjson** jattrsp, NClist** atypesp)
{
	...
	/* Get _nczarr_attrs from .zattrs */
        stat = NCJdictget(jattrs,NCZ_V3_ATTR,&jncattr);
	if(!stat && jncattr == NULL)
	    {stat = NCJdictget(jattrs,NCZ_V3_ATTR_UC,&jncattr);}
	nullfree(key); key = NULL;
	switch (stat) {
	case NC_NOERR: break;
