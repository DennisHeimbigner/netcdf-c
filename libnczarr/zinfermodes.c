...
Also infer mode values such as xarray|noxarray and zarr|nczarr.
...
3.3 Otherwise probe the S3 store to see if we can find significant objects.
*/
...
/**
Probe for an s3 dataset and see if we can infer what it is.
1. Assume an s3 dataset and probe for these top-level objects:
    a. .zgroup | .zarray | .zattrs
	- if found, then infer mode=zarr + version (should be v2).
    c. .zattrs
	- if found then read look for _nczarr_superblock. If found, then
          infer mode=nczarr (and version).
*/

static int
NCZ_infer_s3_format(NC_FILE_INFO_T* file, NCURI* url, ?)
{
