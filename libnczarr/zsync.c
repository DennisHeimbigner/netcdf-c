...
*/
int
ncz_read_atts(NC_FILE_INFO_T* file, NC_OBJ* container, struct ZJSON* jsonz)
{
    int stat = NC_NOERR;
    size_t i;
    char* fullpath = NULL;
    ...
    if(jsonz->jatts != NULL) {
        if((stat = NCZF_decode_attributes_json(file,(NC_OBJ*)container,jsonz->jatts,&jtypes))) goto done;
    }
