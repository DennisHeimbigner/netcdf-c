...
static NClist* NCZ_registeredfilters = NULL; /** List<NC_FILTER_CLIENT_HDF5*> */
...
filterlookup(unsigned int id)
{
	...
	NC_FILTER_CLIENT_HDF5* x = nclistget(NCZ_registeredfilters,i);
...
int
NCZ_hdf5_addfilter(NC_VAR_INFO_T* var, int active, unsigned int id, size_t nparams, unsigned int* inparams)
{
    int stat = NC_NOERR;
...
NCZ_global_filter_action(int op, unsigned int id, NC_FILTER_OBJ_HDF5* infop)
{
	...
	if(infop == NULL) goto done;
   	...
   	if((pos = filterlookup(id)) < 0)
	    {stat = NC_ENOFILTER; goto done;} /* Not defined */
        elem = (NC_FILTER_CLIENT_HDF5*)nclistget(NCZ_registeredfilters,pos);
...
#endif
	if((stat = NCZ_hdf5_addfilter(var,!FILTERACTIVE,id,nparams,params)))
  	    goto done;
#ifdef USE_PARALLEL
...
void
NCZ_freefilterspec(NC_FILTER_SPEC_HDF5* f)
{
    if(f) {
