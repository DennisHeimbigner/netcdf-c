...
int
NCZ_plugin_path_list(int ncid, size_t* npaths, char** pathlist)
{
    int stat = NC_NOERR;
    NC* nc = NULL;
...
EXTERNL int
nc_plugin_path_initialize(int ncid, const char* paths)
{
    int stat = NC_NOERR;
    NC* ncp;
    ...
    stat = NC_check_id(ncid,&ncp);
    if(stat != NC_NOERR) return stat;
    if((stat = ncp->dispatch->plugin_path_initialize(ncid,paths))) goto done;
done:
    return stat;
}
