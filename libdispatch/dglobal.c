...
{
    NC_clearawsparams(aws);
    aws->region = nulldup(NC_rclookupx(uri,AWS_ENV_REGION));
    aws->default_region = nulldup(NC_rclookupx(uri,AWS_ENV_DEFAULT_REGION));
    aws->access_key_id = nulldup(NC_rclookupx(uri,AWS_ENV_ACCESS_KEY_ID));
    aws->secret_access_key = nulldup(NC_rclookupx(uri,AWS_ENV_SECRET_ACCESS_KEY));
    aws->session_token = nulldup(NC_rclookupx(uri,AWS_ENV_SESSION_TOKEN));
    aws->profile = nulldup(NC_rclookupx(uri,AWS_ENV_PROFILE));
    aws->config_file = nulldup(NC_rclookupx(uri,AWS_ENV_CONFIG_FILE));
}
